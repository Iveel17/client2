...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        console.log('Logged in user:', result.user); // Debug user role
        setSuccessMessage(`Welcome, ${result.user.role || 'User'}! Redirecting...`);
        // Persist user to localStorage (handled in AuthProvider, but verify)
        localStorage.setItem('user', JSON.stringify(result.user));
        setTimeout(() => navigate('/'), 1500); // Delay redirect for feedback
      } else {
        ...
        <div className="w-full max-w-4xl bg-white rounded-xl shadow-2xl p-6 sm:p-8 md:p-12 lg:p-16 transform transition-all duration-300 hover:scale-[1.01]">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full py-3 px-4 rounded-lg font-semibold text-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-300 ease-in-out transform shadow-lg flex items-center justify-center ${
                isLoading
          ...
          <div className="mt-8 text-center text-gray-600">
            <p>
              ...
              <Link
                to="/signup"
                className="font-semibold text-blue-600 hover:text-blue-800 transition duration-200 ease-in-out"
              >
                Sign Up
              </Link>
            </p>
